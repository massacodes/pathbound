...
function Navbar() {
  ...
  // Close mobile menu on Escape key press and prevent background scrolling when menu is open
  useEffect(() => {
    if (!isMenuOpen) {
      return;
    }
    ...
    <nav className="fixed z-50 w-full border-b border-slate-100 bg-white/90 backdrop-blur-md">
      ...
      >
        <div
          ...
          role="dialog"
          aria-modal="true"
          className={`absolute right-4 top-24 w-[min(24rem,calc(100%-2rem))] rounded-2xl border border-slate-100 bg-white p-5 shadow-2xl transition-all duration-300 ${
            isMenuOpen
